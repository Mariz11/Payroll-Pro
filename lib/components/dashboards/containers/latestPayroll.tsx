...
function LatestPayroll() {
  ...
  const fetchPayroll = async () => {
    try {
      const response = await fetch(`/api/employeeDashboard/latestPayroll`, {
        method: 'GET',
        headers: {
  ...
  function downloadPayroll() {
    if (!latestPayroll.isLoading && latestPayroll.data.message.length > 0) {
      const adjustment =
        latestPayroll.data.message[0].addAdjustment -
        latestPayroll.data.message[0].deductAdjustment;
      const totalPay = latestPayroll.data.message[0].grossPay + adjustment;
      const workbook = new ExcelJS.Workbook();

      const philhealth = latestPayroll.data.message[0].philhealthContribution;

      // GET SSS LOANS
      const sssLoans = latestPayroll.data.message[0].payroll_deductions.map(
      ...
      // GET CASH ADVANCE
      const cashAdvances = latestPayroll.data.message[0].payroll_deductions.map(
        (payrollDeduction: any) => {
          if (
            payrollDeduction.deduction.deductionType === 'Cash Advance' &&
            payrollDeduction.deduction.isPosted === true
            // && payrollDeduction.deduction.transfer_to_employee_acct_transaction.disbursementStatus === true
          ) {
      ...
      // GET HDMF LOANS
      const HDMFLoans = latestPayroll.data.message[0].payroll_deductions.map(
        (payrollDeduction: any) => {
          if (
      ...
      const earnings = totalPay - latestPayroll.data.message[0].totalDeduction;
      ...
      const earningsData = [
        ...
        ['Payroll Month', `${latestPayroll.data.message[0].businessMonth}`],
        ['', ''],
        ['PAY DESCRIPTION', 'TOTAL'],
        ['Gross Pay', `${latestPayroll.data.message[0].grossPay}`],
        ['Adjustments', `${adjustment}`],
        ['TOTAL PAY', `${totalPay}`],
        ['', ''],
        ['DEDUCTIONS', ``],
        ['DESCRIPTION', `AMOUNT`],
        ['SSS', `${latestPayroll.data.message[0].sssContribution}`],
        ['PHILHEALTH', `${philhealth}`],
        ['HDMF', `${latestPayroll.data.message[0].pagIbigContribution}`],
        ['CASH ADVANCE', `${cashAdvance}`],
        ['SSS LOANS', `${sssLoan}`],
        ['HDMF LOANS', `${HDMFLoan}`],
        ['WITHHOLDING TAX', `${latestPayroll.data.message[0].withholdingTax}`],
        ['TOTAL DEDUCTION', `${latestPayroll.data.message[0].totalDeduction}`],
        ['', ''],
        ['EARNINGS', `${earnings}`],
      ];
      ...
      workbookBuffer.then((buffer) => {
        ...
        a.href = url;
        a.download = `${latestPayroll.data.message[0].businessMonth}_Payroll.xlsx`;
        a.click();
      });
    } else {
      // console.log('test');
  ...
  const [latestPayroll, employeeQuery] = useQueries({
    queries: [
      {
      ...
      },
      {
        ...
        queryFn: async () => {
          const response = await axios.get(
            `/api/employeeDashboard/employeeData`,
            {
              headers: {
                Authorization: `Bearer ${process.env.NEXT_PUBLIC_JWT}`,
              },
  ...
  return (
    <div className="rounded-lg doughnut px-7 w-full md:w-[49.5%]   ">
      <Toast ref={toast} position="bottom-left" />
      <div className="flex flex-col gap-6 py-2">
        <h1 className="text-[20px] px-4 pt-4">Latest Payroll</h1>
        <div className="flex flex-row justify-between p-4 items-center">
          <div className="flex flex-col text-[18px]">
            {latestPayroll.data?.message[0]?.businessMonthCycle ??
              'No Latest Payroll to display'}
          </div>
        </div>
        <div className="login w-full justify-end flex">
          <Button
            label="Download"
            type="submit"
            className="p-button px-16"
            // onClick={downloadPayroll}
            onClick={async () => {
              const data = latestPayroll.data?.message[0];
            ...
            disabled={
              latestPayroll.data?.message[0]?.businessMonthCycle ? false : true
            }
          />
        </div>
      </div>
              ...
              >
                ...
                >
                  {payrollReports.map((data: any, index: number) => {
                    ...
                    const salaryLoanSum = data.payroll_deductions
                      .filter((deduc: any) => {
                        // Add conditions for filtering
                        return (
                        ...
                        >
                          <View style={{ ...styles.row, gap: 4 }}>
                            ...
                            >
                              ...
                              {/* <View style={styles.paragrah}>
                                <Text>
                                  Night Differential ({data.nightDiffHrs} hr
                                  {data.nightDiffHrs > 1 ? 's' : ''}):
                                </Text>
                                <Text>
                                  PHP{' '}
                                  {amountFormatter(data.nightDiffPay) || 0.0}
                              ...
                              <View style={styles.paragrah}>
                                ...
                                <Text>
                                  PHP{' '}
                                  {amountFormatter(
                                    data.addAdjustment - data.deductAdjustment
                                  ) || 0.0}
                                </Text>
                              </View>
                            ...
                            >
                              ...
                              <View style={styles.paragrah}></View>
                              {/* <View style={styles.paragrah}>
