...
const DirectPayrollDownloadSidebar = ({
  ...
  const handleDownloadTemplate = async () => {
    ...
    const response = await axios.post(
      `/api/payrolls/directupload/download`,
      {
        selectedDepartmentId: selectedDepartment.code,
        businessMonth: `${moment(formDataDownload.businessMonth).format(
          'MMMM'
        )} ${moment(formDataDownload.businessMonth).format('YYYY')}`,
        cycle: cycle,
      ...
      setDirectPayrollData(
        response.data.employees.map((i: any) => {
          return {
            ...
            businessYear: moment(formDataDownload.businessMonth).format('YYYY'),
            cycle: cycle,
            daysWorked: 0,
            workingDays: 0,
            netPay: '0.00',
  ...
  const onInputChangeDownload = async (fieldName: string, value: string) => {
    ...
    if (fieldName == 'departmentId') {
      ...
      setSelectedDepartmentName(departmentDetails.name); // for filename
      setPayrollType(departmentDetails.others.payroll_type);
    ...
    if (
      ...
      if (payrollType && payrollType.type) {
        const { company_pay_cycles, type } =
          selectedDepartment.others.payroll_type;
