  ...
  for (let i = 0; i < companyDetails.length; i++) {
    ...
    for (let j = 0; j < employees.length; j++) {
      ...
      if (duration >= employeeTenureship) {
        const payrolls: any = [];
        for (let b = 0; b < businessMonths.length; b++) {
          const getPayrolls = await Payroll.findAll({
            attributes: [
              'businessMonth',
              [Sequelize.literal('SUM(netPay)'), 'netPay'],
            ],
            where: {
              employeeId: employeeId,
              // departmentId: departmentId,
              businessMonth: businessMonths[b],
              isPosted: 1,
            },
            raw: true,
            group: ['businessMonth'],
          ...
          payrolls.push(...getPayrolls);
        }

        // Marked as not qualified if number of payrolls is not equal to qualifyingTerm
        if (payrolls.length != qualifyingTerm) {
          continue;
        }

        const monthWithLowestNetPay = payrolls.sort((a: any, b: any) =>
